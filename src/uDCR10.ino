...
void setup()
{
  // make sure the builtin LED is off
...
void loop()
{
  ...
  if (agc_pwm_temp<116)
  {
    // if the AGC goes below 1.5V then limit this to 1.4V
    agc_pwm_temp = 108;
  }
  // if the volume is still less than the AGC then the volume sets the value
  ...
  switch (state)
  {
    ...
    case STATE_BUTTON_PRESS:
    {
      ...
      if (press_time>LONG_PRESS_TIME)
      {
        ...
        switch (radio.band)
        {
          case BAND_80M: radio.band = BAND_40M; radio.frequency = 7100000u;  break;
